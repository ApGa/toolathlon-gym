...
Copy arXiv paper IDs, titles, full author names, and years from the retrieved records. For Scholar results without a paper ID, use the publication URL or a stable local identifier. Include the source primary category when available (additional source categories are allowed); otherwise provide a topical category. If the source provides no citation count, record Citation_Count as 0 instead of inventing a count. Qualitative methods and gap analysis may reflect your own supported conclusions.
