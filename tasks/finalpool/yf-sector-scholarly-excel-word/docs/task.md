...
Record the selected ticker symbols in financial_data.json (stock-info objects with a symbol field, or an object keyed by ticker). Compute each sector's Avg_Price from currentPrice (regularMarketPrice if unavailable), and Total_Market_Value as the sum of marketCap for those selected stocks. State your volatility method and stock selection in the report; no unique volatility estimator or investment outlook is prescribed. Cite papers actually available from the research tools and distinguish findings from your own interpretation.
